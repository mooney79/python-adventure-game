...
class Game():
    ...
    def play_game(self):
        ...
        basement = Basement()
        basement.lock_solution = basement.get_lock_solution()
        basement.entrance_dark_first()
...
class Basement(Room):
    def __init__(self):
        ...
        self.gas_taken = False 
        self.brick_count = 0
        self.wrench_taken = False
    ...
    def check_time(self):
        if self.timecounter % 10 == 0:
            print()
            self.prRed("The sounds of moaning and shuffling inch a bit closer!  You'd better hurry!")
            if player.weapon != None:
                print(f"You grip your {player.weapon} tightly, hoping against hope that it will help you should they reach you.")
            print()


    def pick_random_area(self, current_area):
        next_area = randint(1,5)
        if next_area == current_area:
            self.pick_random_area(current_area)
        else:
            if next_area == 1:
                self.entrance_dark_subsequent()
            elif next_area == 2:
                self.light_cord_dark()
            elif next_area == 3:
                self.generator_dark()
            elif next_area == 4:
                self.gasoline_dark()
...
closer.  There's no way out but through.""")
    ...
    def entrance_dark_subsequent(self):
        next_step = False
        print()
        print('/////////////////////////////////////////////')
        print('IN THE DARK')
        print('/////////////////////////////////////////////')
        print()
        print("You stub your toes against the base of the stairs.  How did you get back here?")    
        while next_step == False:
            self.timecounter += 1
            self.check_time()
...
you realize that it is a dangling light cord. 
    ...
    def gasoline_dark(self):
        ...
        print('/////////////////////////////////////////////')
        print()
        if player.has_gas or self.gas_taken:
...
in the (h)ole, or (l)eave this area?""")
            if area_selection.lower() == "h":
                ...
                if self.brick_count < 3:
                    ...
                    next_step = True
                    self.secret_room(player)
            elif area_selection.lower() == "i":
                print("A wiseguy, huh?")  
            else:
                print('Please enter one of the (i)ndicated options.')

    def secret_room(self, player):
        next_step = False
        print()
        print('/////////////////////////////////////////////')
        print('SECRET ROOM')
...
''')
        while next_step == False:
            ...
            elif area_selection.lower() == "p" and player.has_pick:
                next_step = True
    ...
    def freedom(self, player):
        print(f"You emerge into the chill October night air!  You've managed to escape!  The basement area took you {self.timecounter} turns, {player.name}.")
        print("""             
                                ,.        ,.      ,.
                                ||        ||      ||  ()
,--. ,-. ,.,-.  ,--.,.,-. ,-.  ||-.,.  ,.|| ,-.  ||-.,. ,-. ,.,-.  ,--.
        ...
        while locked_one == True:
            ...
            if int(pick) in valid_guesses:
                if int(pick) in guessed:
                    ...
                    if int(pick) == self.lock_solution['first']:
        ...
        while locked_two == True:
            ...
            if int(pick) in valid_guesses:
                if int(pick) in guessed:
                    ...
                    if int(pick) == self.lock_solution['second']:
                        print('The pin clicks into place, held there by the tension on the lock.')
                        pin_select = int(pick) - 7
                        keyway = keyway[:pin_select] + "'" + keyway[pin_select + 1:]
                        locked_two = False
        ...
        while locked_three == True:
            ...
            if int(pick) in valid_guesses:
                if int(pick) in guessed:
                    ...
                    if int(pick) == self.lock_solution['third']:
                        print('The pin clicks into place, held there by the tension on the lock.')
                        pin_select = int(pick) - 7
                        keyway = keyway[:pin_select] + "'" + keyway[pin_select + 1:]
                        locked_three = False
        ...
        while locked_four == True:
            ...
            if int(pick) in valid_guesses:
                if int(pick) in guessed:
                    ...
                    if int(pick) == self.lock_solution['fourth']:
                        print('The pin clicks into place, held there by the tension on the lock.')
                        pin_select = int(pick) - 7
                        keyway = keyway[:pin_select] + "'" + keyway[pin_select + 1:]
                        locked_four = False
        ...
        while locked_five == True:
            ...
            if int(pick) in valid_guesses:
                if int(pick) in guessed:
                    ...
                    if int(pick) == self.lock_solution['fifth']:
                        print('The pin clicks into place, held there by the tension on the lock.')
                        pin_select = int(pick) - 7
                        keyway = keyway[:pin_select] + "'" + keyway[pin_select + 1:]
                        locked_five = False
